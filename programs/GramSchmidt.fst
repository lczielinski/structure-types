...
assume val norm (#n:pos) (v:cvec n) : num
...
assume val residual_mat (#n:pos) (#k:pos) (v:cvec n{unit_vec v}) (m1:rect n k{linind m1 /\ mat_vec_ind m1 v}) : 
  m2:rect n k{mat_vec_ortho m2 v /\ linind m2 /\ same_col_span (RectK m1 v) (RectK m2 v)}
// why is m2 still linind
...
let rec gramschmidt_r (#n:pos) (#k:pos) (m:rect n k{linind m}) : 
  (q:rect n k{ortho q /\ same_col_span m q}) & (r:mat k{upper r}) = 
  match m with 
  | Rect1 v -> 
    let q = Rect1 (normalize v) in 
    let r = Mat1 (norm v) in 
    (q, r)
  | RectK m' v -> 
    // assert mat_vec_ind m' v;
    let q' = gramschmidt m' in
